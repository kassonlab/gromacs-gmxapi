...
/// Proxy to execution context.
/*! A proxy can be configured with information needed to initialize a runtime
 * environment capable of executing a work load, independently of defining the work. The actual execution
 * environment is not necessarily instantiated / configured until the work is
 * performed. Thus, construction of a Context object does not necessarily imply
 ...
 * library and documented separately, and it should be straight-forward to
...
class Context
{
    public:
        ...
        /// Bind to a runner.
        Status setRunner(std::shared_ptr<Runner> runner);

        /// Get a handle to the Runner.
        std::shared_ptr<Runner> runner();

        //static std::unique_ptr<Context> fromInputRec(const MDInput &inputrec);
